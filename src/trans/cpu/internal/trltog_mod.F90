...
IF (NTRANS_SYNC_LEVEL <= 0) THEN
  !...Receive loop.........................................................
  DO INR=1,YDBUFS%INRECV
    IRECV=YDBUFS%IRECV(INR)
    CALL MPL_RECV(ZCOMBUFR(-1:YDBUFS%IRECVTOT(IRECV),INR), KSOURCE=NPRCIDS(IRECV), &
...
IF(YDBUFS%IRECVTOT(MYPROC) > 0 )THEN
...
ISEND_FLD_START = 1
ISEND_FLD_END   = KF_FS
DO INS=1,YDBUFS%INSEND
  ISEND=YDBUFS%ISEND(INS)
  ILEN = YDBUFS%ISENDTOT(ISEND)/KF_FS
  !$OMP PARALLEL DO SCHEDULE(STATIC) PRIVATE(JFLD,JL,II)
  DO JL=1,ILEN
    II = YDBUFS%IINDEX(YDBUFS%INDOFF(ISEND)+JL)
  ...
  IF (NTRANS_SYNC_LEVEL <= 1) THEN
    CALL MPL_SEND(ZCOMBUFS(-1:YDBUFS%ISENDTOT(ISEND),INS), KDEST=NPRCIDS(ISEND), &
...
DO JNR=1,YDBUFS%INRECV

  IF (NTRANS_SYNC_LEVEL <= 0) THEN
    CALL MPL_WAITANY(KREQUEST=IREQ_RECV(1:YDBUFS%INRECV), KINDEX=INR, &
...
IF (NTRANS_SYNC_LEVEL <= 1) THEN
  IF(YDBUFS%INSEND > 0) THEN
    CALL MPL_WAIT(KREQUEST=IREQ_SEND(1:YDBUFS%INSEND),CDSTRING='TRLTOG_COMM: WAIT FOR ISENDS')
  ENDIF
ENDIF
