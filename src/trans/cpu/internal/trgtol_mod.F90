...
IF(YDBUFS%ISENDTOT(MYPROC) > 0 )THEN
...
IF (NTRANS_SYNC_LEVEL <= 1) THEN
  IF(YDBUFS%INSEND > 0) THEN
    CALL MPL_WAIT(KREQUEST=IREQ_SEND(1:YDBUFS%INSEND),CDSTRING='TRGTOL_COMM: WAIT FOR ISENDS')
  ENDIF
ENDIF
