...
SUBROUTINE TRGL_INIT_VARS(YLVARS, KF_SCALARS_G, PGP, PGPUV, PGP3A, PGP3B, PGP2)
  ...
  IF (PRESENT(PGPUV)) THEN
    ...
    IF(LVORGP) THEN
      IUVPAR=IUVPAR+1
      DO J=1,IUVLEV
        YLVARS%IUVLEVS(IOFF+J)=J
        YLVARS%IUVPARS(IOFF+J)=IUVPAR
        YLVARS%LLUV(IOFF+J)=.TRUE.
      ENDDO
      IOFF=IOFF+IUVLEV
    ENDIF
    IF(LDIVGP) THEN
    ...
    DO J=1,IUVLEV
      YLVARS%IUVLEVS(IOFF+J)=J
      YLVARS%IUVPARS(IOFF+J)=IUVPAR+1
      YLVARS%IUVLEVS(IOFF+J+IUVLEV)=J
      YLVARS%IUVPARS(IOFF+J+IUVLEV)=IUVPAR+2
    ENDDO
    IUVPAR=IUVPAR+2
    YLVARS%LLUV(IOFF+1:IOFF+2*IUVLEV)=.TRUE.
    IOFF=IOFF+2*IUVLEV
    IOFF1=IOFF
    IOFFNS=IOFFNS+IOFF
    IOFFEW=IOFFEW+IOFF
    ...
    IF(LUVDER) THEN
      ...
      DO J=1,IUVLEV
        ...
        YLVARS%IUVLEVS(IOFF+J+IUVLEV)=J
        YLVARS%IUVPARS(IOFF+J+IUVLEV)=IUVPAR+2
        YLVARS%LLUV(IOFF+J+IUVLEV)=.TRUE.
      ENDDO
      IUVPAR=IUVPAR+2
      IOFF=IOFF+2*IUVLEV
  ...
  IF (PRESENT(PGP2)) THEN
    ...
    DO J=1,IGP2PAR
      YLVARS%LLGP2(J+IOFF) = .TRUE.
      YLVARS%IGP2PARS(J+IOFF)=J
    ENDDO
    IOFF1=IOFF1+IGP2PAR
    IF(LSCDERS) THEN
      IOFF=IOFFNS
      DO J=1,IGP2PAR
        YLVARS%LLGP2(J+IOFF) = .TRUE.
        YLVARS%IGP2PARS(J+IOFF)=J+IGP2PAR
      ENDDO
      IOFFNS=IOFF+IGP2PAR
      IOFF=IOFFEW
      DO J=1,IGP2PAR
        YLVARS%LLGP2(J+IOFF) = .TRUE.
        YLVARS%IGP2PARS(J+IOFF)=J+2*IGP2PAR
      ENDDO
      IOFFEW=IOFF+IGP2PAR
    ENDIF
  ENDIF
  ...
  IF (PRESENT(PGP3A)) THEN
    ...
    DO J1=1,IGP3APAR
      DO J2=1,IGP3ALEV
        YLVARS%LLGP3A(J2+(J1-1)*IGP3ALEV+IOFF) = .TRUE.
        YLVARS%IGP3APARS(J2+(J1-1)*IGP3ALEV+IOFF)=J1
        YLVARS%IGP3ALEVS(J2+(J1-1)*IGP3ALEV+IOFF)=J2
      ENDDO
    ENDDO
    IPAROFF=IGP3APAR
    ...
    IF(LSCDERS) THEN
      ...
      DO J1=1,IGP3APAR
        DO J2=1,IGP3ALEV
          YLVARS%LLGP3A(J2+(J1-1)*IGP3ALEV+IOFF) = .TRUE.
          YLVARS%IGP3APARS(J2+(J1-1)*IGP3ALEV+IOFF)=J1+IPAROFF
          YLVARS%IGP3ALEVS(J2+(J1-1)*IGP3ALEV+IOFF)=J2
        ENDDO
      ENDDO
  ...
  IF (PRESENT(PGP3B)) THEN
    ...
    DO J1=1,IGP3BPAR
      DO J2=1,IGP3BLEV
        YLVARS%LLGP3B(J2+(J1-1)*IGP3BLEV+IOFF) = .TRUE.
        YLVARS%IGP3BPARS(J2+(J1-1)*IGP3BLEV+IOFF)=J1
        YLVARS%IGP3BLEVS(J2+(J1-1)*IGP3BLEV+IOFF)=J2
      ENDDO
    ENDDO
    IPAROFF=IGP3BPAR
    ...
    IF(LSCDERS) THEN
      ...
      DO J1=1,IGP3BPAR
        DO J2=1,IGP3BLEV
          YLVARS%LLGP3B(J2+(J1-1)*IGP3BLEV+IOFF) = .TRUE.
          YLVARS%IGP3BPARS(J2+(J1-1)*IGP3BLEV+IOFF)=J1+IPAROFF
          YLVARS%IGP3BLEVS(J2+(J1-1)*IGP3BLEV+IOFF)=J2
        ENDDO
      ENDDO
...
SUBROUTINE TRGL_INIT_OFF_VARS(YDBUFS,YLVARS,KVSET,KPTRGP,KF_GP)
  ...
  DO JFLD=1,KF_GP
    IF(KVSET(JFLD) == MYSETV .OR. KVSET(JFLD) == -1) THEN
      YDBUFS%IFLDS = YDBUFS%IFLDS+1
      IF(YDBUFS%LLINDER) THEN
        YLVARS%IFLDOFF(YDBUFS%IFLDS) = KPTRGP(JFLD)
      ELSE
        YLVARS%IFLDOFF(YDBUFS%IFLDS) = JFLD
      ENDIF
    ENDIF
  ENDDO
  ...
  IPOS=0
  DO JBLK=1,NGPBLKS
    YLVARS%IGPTROFF(JBLK)=IPOS
    IFIRST = YDBUFS%IGPTRSEND(1,JBLK,MYSETW)
    IF(IFIRST > 0) THEN
      ILAST = YDBUFS%IGPTRSEND(2,JBLK,MYSETW)
...
SUBROUTINE TGRL_INIT_PACKING_VARS(YDBUFS,YLVARS, KVSET, KF_GP, PCOMBUFS)
  ...
  DO INRS=1,KINRS
    ...
    IFLD = 0
    IPOS = 0
    YDBUFS%IPOSPLUS(INRS)=0
    DO JFLD=1,KF_GP
      IF(KVSET(JFLD) == ISETV .OR. KVSET(JFLD) == -1 ) THEN
        IFLD = IFLD+1
        YDBUFS%IFLDA(IFLD,INRS)=JFLD
      ENDIF
    ENDDO

    DO JBLK=1,NGPBLKS
      IFIRST = YDBUFS%IGPTRSEND(1,JBLK,YDBUFS%ISETW(INRS))
      IF(IFIRST > 0) THEN
        ILAST = YDBUFS%IGPTRSEND(2,JBLK,YDBUFS%ISETW(INRS))
        YDBUFS%IJPOS(JBLK,INRS)=IPOS
        YDBUFS%IPOSPLUS(INRS)=YDBUFS%IPOSPLUS(INRS)+(ILAST-IFIRST+1)
        IPOS=IPOS+(ILAST-IFIRST+1)
      ENDIF
    ENDDO
  ...
  DO JJ=I_FLD_START,I_FLD_END
    ...
    DO JBLK=1,NGPBLKS
      ...
      IF(IFIRST > 0) THEN
        ...
        IF(YDBUFS%LLINDER) THEN
          DO JK=IFIRST,ILAST
            JI=(JJ-I_FLD_START)*IPOS+YDBUFS%IJPOS(JBLK,INRS)+JK-IFIRST+1
            IF (YDBUFS%LLTRGTOL) THEN
        ...
        ELSEIF(YDBUFS%LLPGPONLY) THEN
          DO JK=IFIRST,ILAST
            ...
            ENDIF
          ENDDO
        ELSEIF(YLVARS%LLUV(IFLDT)) THEN
          DO JK=IFIRST,ILAST
            JI=(JJ-I_FLD_START)*IPOS+YDBUFS%IJPOS(JBLK,INRS)+JK-IFIRST+1
            IF (YDBUFS%LLTRGTOL) THEN
              ZCOMBUF(JI,INRS) = PGPUV(JK,YLVARS%IUVLEVS(IFLDT),YLVARS%IUVPARS(IFLDT),JBLK)
            ELSE
              PGPUV(JK,YLVARS%IUVLEVS(IFLDT),YLVARS%IUVPARS(IFLDT),JBLK) = ZCOMBUF(JI,INRS)
            ENDIF
          ENDDO
        ELSEIF(YLVARS%LLGP2(IFLDT)) THEN
          DO JK=IFIRST,ILAST
            JI=(JJ-I_FLD_START)*IPOS+YDBUFS%IJPOS(JBLK,INRS)+JK-IFIRST+1
            IF (YDBUFS%LLTRGTOL) THEN
              ZCOMBUF(JI,INRS) = PGP2(JK,YLVARS%IGP2PARS(IFLDT),JBLK)
            ELSE
              PGP2(JK,YLVARS%IGP2PARS(IFLDT),JBLK) = ZCOMBUF(JI,INRS)
            ENDIF
          ENDDO
        ELSEIF(YLVARS%LLGP3A(IFLDT)) THEN
          DO JK=IFIRST,ILAST
            JI=(JJ-I_FLD_START)*IPOS+YDBUFS%IJPOS(JBLK,INRS)+JK-IFIRST+1
            IF (YDBUFS%LLTRGTOL) THEN
              ZCOMBUF(JI,INRS) = PGP3A(JK,YLVARS%IGP3ALEVS(IFLDT),YLVARS%IGP3APARS(IFLDT),JBLK)
            ELSE
              PGP3A(JK,YLVARS%IGP3ALEVS(IFLDT),YLVARS%IGP3APARS(IFLDT),JBLK) = ZCOMBUF(JI,INRS)
            ENDIF
          ENDDO
        ELSEIF(YLVARS%LLGP3B(IFLDT)) THEN
          DO JK=IFIRST,ILAST
            JI=(JJ-I_FLD_START)*IPOS+YDBUFS%IJPOS(JBLK,INRS)+JK-IFIRST+1
            IF (YDBUFS%LLTRGTOL) THEN
              ZCOMBUF(JI,INRS) = PGP3B(JK,YLVARS%IGP3BLEVS(IFLDT),YLVARS%IGP3BPARS(IFLDT),JBLK)
            ELSE
              PGP3B(JK,YLVARS%IGP3BLEVS(IFLDT),YLVARS%IGP3BPARS(IFLDT),JBLK) = ZCOMBUF(JI,INRS)
            ENDIF
          ENDDO
        ENDIF
      ENDIF
...
SUBROUTINE TGRL_COPY_PGLAT(PGLAT, YDBUFS, YLVARS, PGP, PGPUV, PGP3A, PGP3B, PGP2)
  ...
  DO JBLK=1,NGPBLKS
    ...
    IF(IFIRST > 0) THEN
      ...
      IF(YDBUFS%LLPGPONLY) THEN
        DO JFLD64=1,YDBUFS%IFLDS
          IFLD = YLVARS%IFLDOFF(JFLD64)
          !DIR$ VECTOR ALWAYS
          DO JK=IFIRST,ILAST
            IPOS = YDBUFS%INDOFF(MYPROC)+YLVARS%IGPTROFF(JBLK)+JK-IFIRST+1
            IF (YDBUFS%LLTRGTOL) THEN
              PGLAT(JFLD64,YDBUFS%IINDEX(IPOS)) = PGP(JK,IFLD,JBLK)
            ELSE
              PGP(JK,IFLD,JBLK) = PGLAT(JFLD64,YDBUFS%IINDEX(IPOS))
            ENDIF
          ENDDO
        ENDDO
      ELSE
        DO JFLD64=1,YDBUFS%IFLDS
          IFLD = YLVARS%IFLDOFF(JFLD64)
          IF(YLVARS%LLUV(IFLD)) THEN
            !DIR$ VECTOR ALWAYS
            DO JK=IFIRST,ILAST
              IPOS = YDBUFS%INDOFF(MYPROC)+YLVARS%IGPTROFF(JBLK)+JK-IFIRST+1
              IF (YDBUFS%LLTRGTOL) THEN
                PGLAT(JFLD64,YDBUFS%IINDEX(IPOS)) = PGPUV(JK,YLVARS%IUVLEVS(IFLD),YLVARS%IUVPARS(IFLD),JBLK)
              ELSE
                PGPUV(JK,YLVARS%IUVLEVS(IFLD),YLVARS%IUVPARS(IFLD),JBLK) = PGLAT(JFLD64,YDBUFS%IINDEX(IPOS))
              ENDIF
            ENDDO
          ELSEIF(YLVARS%LLGP2(IFLD)) THEN
            !DIR$ VECTOR ALWAYS
            DO JK=IFIRST,ILAST
              IPOS = YDBUFS%INDOFF(MYPROC)+YLVARS%IGPTROFF(JBLK)+JK-IFIRST+1
              IF (YDBUFS%LLTRGTOL) THEN
                PGLAT(JFLD64,YDBUFS%IINDEX(IPOS)) = PGP2(JK,YLVARS%IGP2PARS(IFLD),JBLK)
              ELSE
                PGP2(JK,YLVARS%IGP2PARS(IFLD),JBLK)=PGLAT(JFLD64,YDBUFS%IINDEX(IPOS))
              ENDIF
            ENDDO
          ELSEIF(YLVARS%LLGP3A(IFLD)) THEN
            !DIR$ VECTOR ALWAYS
            DO JK=IFIRST,ILAST
              IPOS = YDBUFS%INDOFF(MYPROC)+YLVARS%IGPTROFF(JBLK)+JK-IFIRST+1
              IF (YDBUFS%LLTRGTOL) THEN
                PGLAT(JFLD64,YDBUFS%IINDEX(IPOS)) = PGP3A(JK,YLVARS%IGP3ALEVS(IFLD),YLVARS%IGP3APARS(IFLD),JBLK)
              ELSE
                PGP3A(JK,YLVARS%IGP3ALEVS(IFLD),YLVARS%IGP3APARS(IFLD),JBLK)=PGLAT(JFLD64,YDBUFS%IINDEX(IPOS))
              ENDIF
            ENDDO
          ELSEIF(YLVARS%LLGP3B(IFLD)) THEN
            !DIR$ VECTOR ALWAYS
            DO JK=IFIRST,ILAST
              IPOS = YDBUFS%INDOFF(MYPROC)+YLVARS%IGPTROFF(JBLK)+JK-IFIRST+1
              IF (YDBUFS%LLTRGTOL) THEN
                PGLAT(JFLD64,YDBUFS%IINDEX(IPOS)) = PGP3B(JK,YLVARS%IGP3BLEVS(IFLD),YLVARS%IGP3BPARS(IFLD),JBLK)
              ELSE
                PGP3B(JK,YLVARS%IGP3BLEVS(IFLD),YLVARS%IGP3BPARS(IFLD),JBLK)=PGLAT(JFLD64,YDBUFS%IINDEX(IPOS))
              ENDIF
            ENDDO
          ELSE
