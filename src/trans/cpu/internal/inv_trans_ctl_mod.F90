...
USE LTINV_CTL_MOD   ,ONLY : LTINV_CTL,LTINV_CTL_PROC
 ...
 & KVSETUV,KVSETSC)
  ...
  DO JBLK=1,IBLKS
    ...
    IF (PRESENT(FSPGL_PROC)) THEN
      CALL LTINV_CTL_PROC(IF_OUT_LT,IF_UV,IF_SCALARS,IF_SCDERS, &
...
ELSE
  ...
  IF (PRESENT(FSPGL_PROC)) THEN
    CALL LTINV_CTL_PROC(KF_OUT_LT,KF_UV,KF_SCALARS,KF_SCDERS, &
     &PSPVOR=PSPVOR,PSPDIV=PSPDIV,PSPSCALAR=PSPSCALAR,&
