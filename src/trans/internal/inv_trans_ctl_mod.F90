...
SUBROUTINE INV_TRANS_CTL_with_proc(KF_UV_G,KF_SCALARS_G,KF_GP,KF_FS,KF_OUT_LT,&
 & KF_UV,KF_SCALARS,KF_SCDERS,&
...
USE SHUFFLE_MOD     ,ONLY : SHUFFLE
USE FIELD_SPLIT_MOD ,ONLY : FIELD_SPLIT
USE LTINV_CTL_MOD   ,ONLY : LTINV_CTL_with_proc
USE FTINV_CTL_MOD   ,ONLY : FTINV_CTL
!
...
IF(NPROMATR > 0 .AND. IF_GPB > NPROMATR) THEN
 ...
 & KVSETUV,KVSETSC)
  ...
  DO JBLK=1,IBLKS
    ...
    CALL LTINV_CTL_with_proc(IF_OUT_LT,IF_UV,IF_SCALARS,IF_SCDERS, &
     & PSPVOR=PSPVOR,PSPDIV=PSPDIV,PSPSCALAR=PSPSCALAR,&
...
ELSE
  ...
  CALL LTINV_CTL_with_proc(KF_OUT_LT,KF_UV,KF_SCALARS,KF_SCDERS, &
   &PSPVOR=PSPVOR,PSPDIV=PSPDIV,PSPSCALAR=PSPSCALAR,&
   &PSPSC3A=PSPSC3A,PSPSC3B=PSPSC3B,PSPSC2=PSPSC2,&
...
END SUBROUTINE INV_TRANS_CTL_with_proc
