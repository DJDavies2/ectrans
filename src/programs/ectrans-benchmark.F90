...
if (lprint_norms .or. ncheck > 0) then
  ...
  if (verbosity >= 1) then
    do ifld = 1, nflevg
      ...
      write(nout,'("0x",Z16.16)') transfer(znormvor1(ifld),ifld)
    enddo
    do ifld = 1, nflevg
      write(nout,'("norm zspdiv( ",i4,",:)   = ",f20.15)') ifld, znormdiv1(ifld)
      write(nout,'("0x",Z16.16)') transfer(znormdiv1(ifld),ifld)
    enddo
    if (nfld > 0) then
      do ifld = 1, nflevg
        write(nout,'("norm zspsc3a(",i4,",:,1) = ",f20.15)') ifld, znormt1(ifld)
        write(nout,'("0x",Z16.16)') transfer(znormt1(ifld),ifld)
      enddo
    endif
    do ifld = 1, 1
      write(nout,'("norm zspsc2( ",i4,",:)   = ",f20.15)') ifld, znormsp1(ifld)
      write(nout,'("0x",Z16.16)') transfer(znormsp1(ifld),ifld)
    enddo
  endif
endif
  ...
  if (myproc == 1) then
    ...
    do ifld = 1, nflevg
      ...
      if (verbosity >= 1) then
        ...
        write(nout,'("0x",Z16.16)') transfer(znormvor(ifld),ifld)
      endif
    enddo
    do ifld = 1, nflevg
      ...
      if (verbosity >= 1) then
        ...
        write(nout,'("0x",Z16.16)') transfer(znormdiv(ifld),ifld)
      endif
    enddo
    if (nfld > 0) then
      do ifld = 1, nflevg
        ...
        if (verbosity >= 1) then
          ...
          write(nout,'("0x",Z16.16)') transfer(znormt(ifld),ifld)
        endif
      enddo
    endif
    do ifld = 1, 1
      ...
      if (verbosity >= 1) then
        ...
        write(nout,'("0x",Z16.16)') transfer(znormsp(ifld),ifld)
      endif
    enddo
